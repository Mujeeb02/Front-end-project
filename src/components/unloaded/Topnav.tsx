    ...
    return ( 
        ...
        <div className="w-[100%] h-[10vh] text-zinc-200 font-bold relative flex justify-center items-center">
            ...
            <div className="absolute w-[50%] max-h-[50vh] bg-zinc-200 top-[100%] overflow-auto rounded-lg ">
                { searches.map((s,i)=>(
                     <Link  to={`/details/${s.id}`} key={i} className="hover:text-black hover:bg-zinc-400 font-semibold text-zinc-600 w-[100%] p-10 flex justify-start items-center border-2 border-zinc-100">
                        <img className="h-[10vh] w-[10vh] rounded-lg mr-4" src={s.backdrop_path || s.backdrop_path?`https://image.tmdb.org/t/p/w500/${s.backdrop_path || s.profile_path}`:noimage} alt="" />
